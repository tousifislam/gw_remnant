...
def _valid_args():
    ...
    h_dict = {(2, 2): h22}
    return time, h_dict, 2.0, None, None, None
...
class TestTimeValidation:
    def test_time_too_short(self):
        time = np.array([0.0])
        _, h_dict, q, s1, s2, ecc = _valid_args()
        h_dict = {(2, 2): np.array([1.0 + 0j])}
        with pytest.raises(ValueError, match="at least 2 elements"):
            InitialEnergyMomenta._validate_inputs(time, h_dict, q, s1, s2, ecc)

    def test_time_non_monotonic(self):
        time = np.array([0.0, 1.0, 0.5, 2.0])
        _, h_dict, q, s1, s2, ecc = _valid_args()
        h_dict = {(2, 2): np.ones(4, dtype=complex)}
...
class TestModeValidation:
    def test_missing_22_mode(self):
        ...
        h_dict = {(3, 3): np.ones(len(time), dtype=complex)}
...
class TestSpinValidation:
    ...
    def test_spin_magnitude_exceeds_one(self):
        ...
        with pytest.raises(ValueError, match="magnitude must be <= 1"):
            InitialEnergyMomenta._validate_inputs(time, h_dict, q, big_spin, None, ecc)


class TestEccentricityValidation:
    def test_eccentricity_negative(self):
        time, h_dict, q, s1, s2, _ = _valid_args()
        with pytest.raises(ValueError, match="0 <= e < 1"):
            InitialEnergyMomenta._validate_inputs(time, h_dict, q, s1, s2, -0.1)

    def test_eccentricity_one(self):
        time, h_dict, q, s1, s2, _ = _valid_args()
        with pytest.raises(ValueError, match="0 <= e < 1"):
            InitialEnergyMomenta._validate_inputs(time, h_dict, q, s1, s2, 1.0)
...
class TestMInitialValidation:
    def test_negative_initial_mass(self):
        ...
        time, h_dict, q = _valid_args()[:3]
        with pytest.raises(ValueError, match="M_initial must be positive"):
            GWRemnantCalculator(time, h_dict, q, M_initial=-1.0)
